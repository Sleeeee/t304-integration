...
const UserGroupsList: React.FC<UserGroupsListProps> = ({ onNavigate }) => {
	...
	const fetchGroups = async () => {
		...
		try {
			const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/users/groups/`, {
				method: "GET",
				credentials: "include",
				headers,
			});
	...
	const handleConfirmDelete = async () => {
		...
		const headers: HeadersInit = csrfToken ? { "X-CSRFToken": csrfToken } : {};
		try {
			const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/users/groups/${groupToDelete.id}/delete/`, {
				method: "DELETE",
				credentials: "include",
				headers,
			});
