...
const ManageGroupDialog: React.FC<ManageGroupDialogProps> = ({ open, onClose, group, onGroupUpdated }) => {
	...
	useEffect(() => {
		...
		const fetchAllUsers = fetch(`${process.env.REACT_APP_BACKEND_URL}/users/`, {
			method: "GET",
			credentials: "include",
			headers: getHeaders(),
		});

		const fetchGroupMembers = fetch(`${process.env.REACT_APP_BACKEND_URL}/users/groups/${group.id}/users/`, {
			method: "GET",
			credentials: "include",
			headers: getHeaders(),
		});
	...
	const handleUpdateName = async () => {
		...
		try {
			const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/users/groups/${group.id}/update/`, {
				method: 'PATCH',
				credentials: 'include',
	...
	const handleSubmitMembers = async (event: React.FormEvent<HTMLFormElement>) => {
		...
		try {
			...
			if (usersToAdd.length > 0) {
				requests.push(
					fetch(`${process.env.REACT_APP_BACKEND_URL}/users/groups/${group.id}/add_user/`, {
						method: "POST",
						credentials: "include",
						headers: getHeaders(true),
			...
			if (usersToRemove.length > 0) {
				requests.push(
					fetch(`${process.env.REACT_APP_BACKEND_URL}/users/groups/${group.id}/remove_user/`, {
						method: "DELETE",
						credentials: "include",
						headers: getHeaders(true),
